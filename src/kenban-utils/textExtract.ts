 ...
 */
function collectVisibleTextLayers(children: any[], parentVisible = true): ExtractedTextLayer[] {
...
function normalizeConfusables(text: string): string {
  return (
    text
      ...
      .replace(/[\uFF01-\uFF5A]/g, (c) => String.fromCharCode(c.charCodeAt(0) - 0xfee0))
  );
}
...
function computeCharDiff(psdLine: string, memoLine: string): { psd: DiffPart[]; memo: DiffPart[] } {
  ...
  const flushPsd = () => {
    if (psdBuf) {
      psd.push(psdRem ? { value: psdBuf, removed: true } : { value: psdBuf });
      psdBuf = "";
    }
  };
  const flushMemo = () => {
    if (memoBuf) {
      memo.push(memoAdd ? { value: memoBuf, added: true } : { value: memoBuf });
      memoBuf = "";
    }
  };
...
export function computeSharedGroupDiff(
  ...
  const pageCharDiffs: Map<number, { psd: DiffPart[]; memo: DiffPart[] }>[] = allPsdLines.map(
    () => new Map(),
  );
...
export function findBestMemoSection(
  ...
  for (let si = 0; si < sections.length; si++) {
    const memoLines = preNormalized
      ...
      : normalizeTextForComparison(sections[si].text)
          .split("\n")
          .filter((l) => l.length > 0 && l !== CHUNK_BREAK);
...
export function buildUnifiedDiff(psdParts: DiffPart[], memoParts: DiffPart[]): UnifiedDiffEntry[] {
  ...
  while (pi < pLines.length || mi < mLines.length) {
    ...
    while (pi < pLines.length && !pLines[pi].changed && mi < mLines.length && !mLines[mi].changed) {
...
function postProcessChunkBreaks(entries: UnifiedDiffEntry[]): UnifiedDiffEntry[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.type === "diff") {
      const psdIsChunk =
        entry.psdParts && entry.psdParts.every((p) => p.value === CHUNK_BREAK || p.value === "");
      const memoIsChunk =
        entry.memoParts && entry.memoParts.every((p) => p.value === CHUNK_BREAK || p.value === "");
  ...
  for (const entry of result) {
    if (
      ...
      collapsed.length > 0 &&
      collapsed[collapsed.length - 1].type === "separator"
    ) {
      continue;
    }
...
export function getMemoTextFromEntry(entry: UnifiedDiffEntry): string | null {
  switch (entry.type) {
    ...
    case "diff":
      return entry.memoParts?.map((p) => p.value).join("") ?? null;
    case "linebreak":
      return entry.memoText ?? null;
    default:
      return null;
  }
}
...
export function getPsdTextFromEntry(entry: UnifiedDiffEntry): string | null {
  switch (entry.type) {
    case "match":
      return entry.text ?? null;
    case "diff":
      return entry.psdParts?.map((p) => p.value).join("") ?? null;
    case "linebreak":
      return entry.psdText ?? null;
    default:
      return null;
  }
}
