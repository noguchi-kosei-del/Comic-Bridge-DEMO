...
export const checkPdfFileSize = (file: File): boolean => {
  if (file.size > PDF_SIZE_WARNING_THRESHOLD) {
    ...
    alert(
      `このPDFファイルは ${sizeMB}MB で、100MBを超えています。\n別のファイルを選択するか、圧縮処理をしてください。`,
    );
    return false;
  }
  return true;
...
export let globalOptimizeProgress:
  | ((fileName: string, message: string, current?: number, total?: number) => void)
  | null = null;
export function setOptimizeProgressCallback(
  cb: ((fileName: string, message: string, current?: number, total?: number) => void) | null,
) {
...
export class PdfCacheManager {
  ...
  constructor() {
    ...
    this.bitmapCache = new LRUCache<BitmapCacheEntry>(PDF_CACHE_MAX_SIZE, (entry) =>
      entry.bitmap.close(),
    );
  ...
  async getDocument(file: File, enableOptimization = true): Promise<PdfDocCache> {
    ...
    if (PDF_AUTO_OPTIMIZE_ENABLED && enableOptimization) {
      ...
      if (this.compressedDataCache.has(fileId)) {
        ...
        try {
          console.log(
            `[PdfCache] Heavy compression for very large PDF: ${file.name} (${(file.size / 1024 / 1024).toFixed(1)}MB)`,
          );
          arrayBuffer = await compressPdfViaCanvas(arrayBuffer, (current, total) => {
            if (globalOptimizeProgress)
              globalOptimizeProgress(file.name, "PDFを圧縮しています...", current, total);
          });
        ...
        try {
          console.log(
            `[PdfCache] Optimizing PDF: ${file.name} (${(file.size / 1024 / 1024).toFixed(1)}MB)`,
          );
  ...
  async renderSplitPageBitmap(
    ...
    try {
      ...
      const entry: BitmapCacheEntry = {
        bitmap,
        width: splitCanvas.width,
        height: splitCanvas.height,
      };
  ...
  prioritizePage(fileA: File, fileB: File, pageNum: number, totalPages: number) {
    ...
    this.preloadQueue = this.preloadQueue.filter((item) => !priorityPages.includes(item.page));
