...
const DELIMITER_PATTERNS: DelimiterPattern[] = [
  ...
  // <<2Page>>, <<2>>, << 2Page >>, <<2P>>
  {
    regex: /^<<\s*(\d{1,4})\s*(?:Page|[Pp]age|[Pp])?\s*>>\s*$/m,
    extractPage: (m) => parseInt(m[1], 10),
  },
...
export function detectDelimiterPattern(text: string): DelimiterPattern | null {
  ...
  for (const pattern of DELIMITER_PATTERNS) {
    ...
    if (matches.length >= 2) {
      ...
      if (
        matches.length > bestCount ||
        (matches.length === bestCount && bestIsSequential && !isSeq)
      ) {
        bestPattern = pattern;
  ...
  if (hasPreText) {
    ...
    if (pattern.sequential) {
      ...
      sections.push({
        pageNums: [1],
        text: preText,
        rangeStart: preRangeStart,
        rangeEnd: preRangeEnd,
      });
    } else {
...
export function replaceMemoSection(
  ...
  return (
    raw.slice(0, section.rangeStart) +
    leadingWS +
    newText.trim() +
    trailingWS +
