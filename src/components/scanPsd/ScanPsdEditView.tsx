...
export function ScanPsdEditView() {
    ...
    try {
      ...
      if (result.folders.length > 0) {
        detected = result.folders.map((f, i) => ({
          path: f.path, name: f.name, volume: maxVol + 1 + i,
        }));
      } else {
  ...
  const handleDialogAddFolder = async () => {
    ...
    const maxVol = Math.max(
      getMaxExistingVolume(),
      ...pendingFolders.map((f) => f.volume),
      0
    );
    const name = selected.split(/[\\/]/).pop() || selected;
  ...
  const handleDialogVolumeChange = (index: number, volume: number) => {
    setPendingFolders((prev) =>
      prev.map((f, i) => (i === index ? { ...f, volume } : f))
    );
  };
    ...
    <div className="h-full flex flex-col overflow-hidden bg-bg-primary">
      ...
      <div className="px-5 py-2.5 border-b border-border bg-white flex items-center gap-3 flex-shrink-0 shadow-soft">
        ...
        <button
          ...
          disabled={phase !== "idle" || !scanData?.folderVolumeMapping || Object.keys(scanData.folderVolumeMapping).length === 0}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isSuccess ? (
            <svg className="w-10 h-10 mx-auto mb-2 text-success" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          ) : (
            <svg className="w-10 h-10 mx-auto mb-2 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="flex-1 overflow-auto px-5 py-3">
          {folders.length === 0 ? (
            ...
            <div className="space-y-1.5">
              ...
              {folders.map((f, i) => (
                <div key={i} className="flex items-center gap-2 bg-bg-secondary rounded-lg px-3 py-2">
...
function VolumeManageDialog({
  ...
  const handleRemove = async (volume: number) => {
    if (!confirm(`${String(volume).padStart(2, "0")}巻のスキャンデータを削除しますか？\n削除後、追加スキャンで再スキャンできます。`)) return;
    setRemoving(volume);
    try {
          ...
          {sortedVolumes.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {sortedVolumes.map(([vol, group]) => (
                <div key={vol} className="flex items-center gap-3 bg-bg-secondary rounded-lg px-3 py-2.5">
