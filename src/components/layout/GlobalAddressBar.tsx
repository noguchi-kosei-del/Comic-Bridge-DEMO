...
function DataLoadButtons() {
  ...
  return (
    <div className="flex items-center gap-0.5 flex-shrink-0">
      ...
      <Btn loaded={presetsLoaded} label="作品情報" title="作品情報JSON" clearTitle="クリア" cCls="text-accent-secondary hover:bg-accent-secondary/15" bCls="border-accent-secondary/50" onLoad={handleLoadPreset} onClear={() => { useUnifiedViewerStore.getState().setFontPresets([]); useUnifiedViewerStore.getState().setPresetJsonPath(null); }} />
      <Btn loaded={checkLoaded} label="校正JSON" title="校正JSON" clearTitle="クリア" cCls="text-warning hover:bg-warning/15" bCls="border-warning/50" onLoad={handleLoadCheck} onClear={() => useUnifiedViewerStore.getState().setCheckData(null)} />
