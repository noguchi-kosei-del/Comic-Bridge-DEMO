...
export function DiffViewerView({ externalPathA, externalPathB }: Props = {}) {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-bg-primary">
      {/* ════ サイドバー ════ */}
      {!isFullscreen && (
        <div className="w-[272px] flex-shrink-0 flex flex-col border-r border-border bg-bg-secondary">
          {/* 比較モード */}
          ...
          <div className="px-3 py-2 border-b border-border space-y-2">
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex gap-1">
                  <button onClick={() => handleSelectFolder("A")} className="w-5 h-5 flex items-center justify-center text-text-muted hover:text-text-primary bg-bg-tertiary rounded transition-colors" title="フォルダ選択"><FolderOpen className="w-3 h-3" /></button>
                  <button onClick={() => handleSelectFile("A")} className="w-5 h-5 flex items-center justify-center text-text-muted hover:text-text-primary bg-bg-tertiary rounded transition-colors" title="ファイル選択"><FileText className="w-3 h-3" /></button>
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex gap-1">
                  <button onClick={() => handleSelectFolder("B")} className="w-5 h-5 flex items-center justify-center text-text-muted hover:text-text-primary bg-bg-tertiary rounded transition-colors" title="フォルダ選択"><FolderOpen className="w-3 h-3" /></button>
                  <button onClick={() => handleSelectFile("B")} className="w-5 h-5 flex items-center justify-center text-text-muted hover:text-text-primary bg-bg-tertiary rounded transition-colors" title="ファイル選択"><FileText className="w-3 h-3" /></button>
                </div>
              </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {!isFullscreen && <div className="flex-shrink-0 flex items-center gap-1 px-3 py-1.5 border-b border-border bg-bg-secondary">
          ...
          {/* アクション */}
          <button onClick={handleOpenFolder} className="w-6 h-6 flex items-center justify-center text-text-secondary hover:text-text-primary transition-colors" title="フォルダを開く"><FolderOpen className="w-3.5 h-3.5" /></button>
