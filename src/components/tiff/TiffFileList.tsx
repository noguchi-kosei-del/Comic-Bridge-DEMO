...
export function TiffFileList({ onOpenCropEditor }: TiffFileListProps) {
  ...
  // パネル全体の折りたたみ状態（プレビューパネルと同じパターン、デフォルト閉）
  ...
  return (
    <div className={`flex flex-col h-full bg-bg-secondary select-none transition-[width] duration-300 ease-out ${collapsed ? "w-8" : "w-[210px]"}`}>
      {collapsed ? (
        /* 折りたたみ時: 細いストリップ（展開ボタンのみ） */
        <div className="flex-shrink-0 h-8 border-b border-border/50 flex items-center justify-center">
          <button
      ...
      <div className="px-2.5 py-2 border-b border-border flex items-center gap-2 flex-shrink-0">
        <button
          ...
          className="w-5 h-5 flex-shrink-0 flex items-center justify-center text-text-muted hover:text-text-primary rounded hover:bg-bg-tertiary"
          title="ファイルリストを格納"
        >
          {/* 二重シェブロン左向き（パネルを左へ格納） */}
          <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
