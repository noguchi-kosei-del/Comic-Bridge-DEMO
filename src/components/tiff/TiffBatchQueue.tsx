...
function SubfolderHeader({ name, fileCount }: { name: string; fileCount: number }) {
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-folder/10 border-b border-folder/25 sticky top-0 z-10">
      <svg
        className="w-3.5 h-3.5 text-folder flex-shrink-0"
        fill="currentColor"
        viewBox="0 0 24 24"
      >
        <path d="M10 4H4a2 2 0 00-2 2v12a2 2 0 002 2h16a2 2 0 002-2V8a2 2 0 00-2-2h-8l-2-2z" />
      </svg>
      <span className="text-[11px] font-semibold text-folder-dark truncate">{name}</span>
      <span className="text-[10px] text-folder-dark/70 flex-shrink-0">{fileCount}</span>
    </div>
  );
}
