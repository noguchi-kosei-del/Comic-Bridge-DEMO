...
const getModeLabels = (compareMode: CompareMode) => {
  switch (compareMode) {
    case "tiff-tiff":
      return { a: "TIFF/JPG (元)", b: "TIFF/JPG (修正)", accept: ".tif,.tiff,.jpg,.jpeg" };
    case "psd-psd":
    ...
    case "psd-tiff":
      return {
        a: "PSD (元)",
        b: "TIFF/JPG (出力)",
        accept: { a: ".psd", b: ".tif,.tiff,.jpg,.jpeg" },
      };
    default:
...
const DiffViewer: React.FC<DiffViewerProps> = (props) => {
  ...
  // フォルダパスを取得
  const getDiffFolderPath = useCallback(
  ...
  return (
    <div className="flex-1 flex flex-col bg-black relative">
      {/* Toolbar */}
      <div
        className={`bg-neutral-800/80 backdrop-blur-sm border-b border-white/[0.06] flex items-center justify-between z-10 shrink-0 px-3 transition-all duration-300 ease-in-out ${isFullscreen || fullscreenTransitioning ? "h-0 opacity-0 border-b-0 overflow-hidden" : "h-11 opacity-100 overflow-visible"}`}
      >
        <div className="flex items-center gap-1.5 shrink-0 flex-nowrap">
          ...
          <div className="bg-neutral-950 rounded-lg flex p-0.5 gap-0.5">
            <button
              ...
              className={`text-xs rounded-md px-2 py-1 transition-all ${viewMode === "A" ? "bg-neutral-700 text-neutral-100 shadow-sm" : "text-neutral-500 hover:text-neutral-200 disabled:opacity-30"}`}
            >
              {modeLabels.a}
            </button>
            {compareMode === "psd-tiff" && (
              <button
                onClick={() => setViewMode("A-full")}
                disabled={!currentPair || currentPair.status !== "done"}
                className={`px-2 py-1 text-xs rounded-md transition-all ${viewMode === "A-full" ? "bg-neutral-700 text-neutral-100 shadow-sm" : "text-neutral-500 hover:text-neutral-200 disabled:opacity-30"}`}
              >
                全体
              </button>
            )}
            <button
              ...
              disabled={!currentPair || currentPair.status !== "done"}
              className={`text-xs rounded-md px-2 py-1 transition-all ${viewMode === "B" ? "bg-neutral-700 text-neutral-100 shadow-sm" : "text-neutral-500 hover:text-neutral-200 disabled:opacity-30"}`}
            >
              {modeLabels.b}
            </button>
            <button
              ...
              disabled={!currentPair || currentPair.status !== "done"}
              className={`text-xs rounded-md flex items-center gap-1 px-2 py-1 transition-all ${viewMode === "diff" ? "bg-red-900/40 text-red-300 shadow-sm" : "text-neutral-500 hover:text-neutral-200 disabled:opacity-30"}`}
            >
              <FileDiff size={12} />
              差分
            </button>
          ...
          {(compareMode === "psd-psd" || compareMode === "psd-tiff") && (
            <button
              ...
              disabled={
                ...
                viewMode === "diff" ||
                (viewMode === "B" && compareMode === "psd-tiff")
              }
            ...
            >
              <Layers size={12} />
              Photoshop<span className="opacity-50 text-[11px]">[P]</span>
            </button>
          )}
          {compareMode === "pdf-pdf" && (
            <button
              onClick={() => {
                const pdfFile =
                  viewMode === "A" || viewMode === "A-full" || viewMode === "diff"
                    ? (currentPair?.fileA as FileWithPath | null)
                    : (currentPair?.fileB as FileWithPath | null);
                if (pdfFile?.filePath) {
                  releaseMemoryBeforeMojiQ();
            ...
            >
              <FileText size={12} />
              MojiQ<span className="opacity-50 text-[11px]">[Q]</span>
            </button>
          )}
          <button
            onClick={() => {
          ...
          {compareMode !== "pdf-pdf" && (
            <button
              ...
              disabled={
                ...
                currentPair.status === "rendering"
              }
              className="text-xs rounded-md bg-neutral-700 hover:bg-neutral-600 text-neutral-300 disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-1 px-2.5 py-1.5 transition-colors"
              title="ファイルを再読み込み (F5)"
            >
              <RefreshCw size={12} />
              更新<span className="opacity-50 text-[11px]">[F5]</span>
            </button>
          )}
          {/* フォルダを開くボタン（差分モード） */}
          {(() => {
                    ...
                    <div className="absolute top-full left-0 mt-2 p-2 bg-neutral-800 rounded-lg shadow-[0_4px_16px_rgba(0,0,0,0.4)] border border-white/[0.10] z-50 min-w-48">
                      ...
                      >
                        ...
                        <span className="text-neutral-500 truncate">
                          {(filesA[0] as FileWithPath)?.filePath?.split(/[/\\]/).slice(-2, -1)[0] ||
                            "-"}
                        </span>
                      </button>
                      <button
                      ...
                      >
                        ...
                        <span className="text-neutral-500 truncate">
                          {(filesB[0] as FileWithPath)?.filePath?.split(/[/\\]/).slice(-2, -1)[0] ||
                            "-"}
                        </span>
                      </button>
                    </div>
          ...
          >
            <Maximize2 size={12} />
            <span className="opacity-50 text-[11px]">[F11]</span>
          </button>
        </div>
        ...
        <div className="flex items-center text-xs text-neutral-500 gap-1.5 flex-nowrap min-w-0 overflow-hidden shrink">
          {compareMode === "pdf-pdf" &&
            currentPair?.status === "done" &&
            currentPair.totalPages > 1 && (
              <div className="flex items-center gap-1 px-2 py-1 bg-neutral-950 rounded-lg border border-white/[0.06] shrink-0">
                <button
                  onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
                  disabled={currentPage <= 1}
                  tabIndex={-1}
                  className="px-2 py-1 rounded-md hover:bg-white/[0.06] disabled:opacity-30 transition-colors"
                >
                  ◀
                </button>
                <span className="px-2 min-w-[80px] text-center text-neutral-300">
                  {isLoadingPage ? (
                    <Loader2 size={12} className="inline animate-spin" />
                  ) : (
                    <>
                      {currentPage} / {currentPair.totalPages}
                    </>
                  )}
                </span>
                <button
                  onClick={() =>
                    setCurrentPage((prev) => Math.min(prev + 1, currentPair.totalPages))
                  }
                  disabled={currentPage >= currentPair.totalPages}
                  tabIndex={-1}
                  className="px-2 py-1 rounded-md hover:bg-white/[0.06] disabled:opacity-30 transition-colors"
                >
                  ▶
          ...
          >
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 bg-white/[0.06] rounded text-neutral-500 font-mono text-[10px] border border-white/[0.08]">
                Space
              </kbd>
      ...
      {/* Main viewer area */}
      <div
        className={`flex-1 relative ${initialModeSelect ? "overflow-y-auto" : "overflow-hidden"} flex items-center justify-center bg-neutral-950 ${isFullscreen ? "" : "p-4"} transition-colors ${!currentPair && dragOverSide ? "bg-neutral-900" : ""}`}
        onDragOver={handleDragOver}
      >
        {currentPair ? (
          currentPair.status === "loading" || currentPair.status === "rendering" ? (
            <div className="flex flex-col items-center text-action">
              <Loader2 size={48} className="animate-spin mb-4" />
              <p>{currentPair.status === "rendering" ? "画像を生成中..." : "解析中..."}</p>
            </div>
          ) : currentPair.status === "checked" ? (
            <div className="flex flex-col items-center text-action">
              <Loader2 size={48} className="animate-spin mb-4" />
            ...
            <div className="text-red-400 text-center">
              <p>読み込みに失敗しました</p>
              <p className="text-xs text-neutral-600 mt-2">{currentPair.errorMessage}</p>
            </div>
          ) : currentPair.status === "pending" ? (
            <div className="flex flex-col items-center w-full max-w-3xl">
              {currentPair.fileA &&
              currentPair.fileB &&
              (compareMode !== "psd-tiff" || cropBounds) ? (
                <>
                  <Loader2 size={48} className="animate-spin mb-4 opacity-50 text-action" />
                  <p className="text-neutral-600">順番待ち...</p>
                </>
              ) : (
                <>
                  ...
                  <div className="flex gap-4 w-full">
                    {compareMode === "psd-tiff" && (
                      ...
                      >
                        <HardDrive
                          size={36}
                          className={`mb-3 ${dragOverSide === "json" ? "text-orange-400" : cropBounds ? "text-green-400" : "text-neutral-600"}`}
                        />
                        <p
                          className={`text-sm font-medium ${dragOverSide === "json" ? "text-orange-300" : cropBounds ? "text-green-400" : "text-neutral-500"}`}
                        >
                          Gドライブ
                        </p>
                    ...
                    <div
                      ref={dropZoneARef}
                      className={`flex-1 border border-dashed rounded-xl py-40 px-16 min-h-[600px] flex flex-col items-center justify-center transition-all cursor-pointer ${dragOverSide === "A" ? "border-blue-400/50 bg-blue-900/15" : filesA.length > 0 ? "border-green-500/40 bg-green-900/10" : "border-white/[0.08] hover:border-white/[0.15] hover:bg-white/[0.02]"}`}
                      onDragOver={handleDragOver}
                      onDragEnter={handleDragEnter("A")}
                      onDragLeave={handleDragLeave}
                      onDrop={handleDrop("A")}
                      onClick={handleFilesAUpload}
                    >
                      <FolderOpen
                        size={36}
                        className={`mb-3 ${dragOverSide === "A" ? "text-blue-400" : filesA.length > 0 ? "text-green-400" : "text-neutral-600"}`}
                      />
                      <p
                        className={`text-sm font-medium ${dragOverSide === "A" ? "text-blue-400" : filesA.length > 0 ? "text-green-400" : "text-neutral-500"}`}
                      >
                        {modeLabels.a}
                      </p>
                      <p className="text-xs text-neutral-600 mt-1">
                        {getAcceptedExtensions("A", compareMode).join(", ")}
                      </p>
                      {filesA.length > 0 && (
                        <p className="text-xs text-green-400 mt-2">{filesA.length}件</p>
                      )}
                    </div>

                    <div
                      ref={dropZoneBRef}
                      className={`flex-1 border border-dashed rounded-xl py-40 px-16 min-h-[600px] flex flex-col items-center justify-center transition-all cursor-pointer ${dragOverSide === "B" ? "border-green-400/50 bg-green-900/15" : filesB.length > 0 ? "border-green-500/40 bg-green-900/10" : "border-white/[0.08] hover:border-white/[0.15] hover:bg-white/[0.02]"}`}
                      onDragOver={handleDragOver}
                      onDragEnter={handleDragEnter("B")}
                      onDragLeave={handleDragLeave}
                      onDrop={handleDrop("B")}
                      onClick={handleFilesBUpload}
                    >
                      <FolderOpen
                        size={36}
                        className={`mb-3 ${dragOverSide === "B" ? "text-green-400" : filesB.length > 0 ? "text-green-400" : "text-neutral-600"}`}
                      />
                      <p
                        className={`text-sm font-medium ${dragOverSide === "B" ? "text-green-400" : filesB.length > 0 ? "text-green-400" : "text-neutral-500"}`}
                      >
                        {modeLabels.b}
                      </p>
                      <p className="text-xs text-neutral-600 mt-1">
                        {getAcceptedExtensions("B", compareMode).join(", ")}
                      </p>
                      {filesB.length > 0 && (
                        <p className="text-xs text-green-400 mt-2">{filesB.length}件</p>
                      )}
                    </div>
                  </div>
                  ...
                  {compareMode === "psd-tiff" && (
                    <div className="mt-4 flex gap-2 text-xs">
                      <span className={cropBounds ? "text-green-400" : "text-neutral-600"}>
                        1. JSON {cropBounds ? "OK" : "..."}
                      </span>
                      <span className="text-neutral-700">→</span>
                      <span className={filesA.length > 0 ? "text-green-400" : "text-neutral-600"}>
                        2. PSD {filesA.length > 0 ? "OK" : "..."}
                      </span>
                      <span className="text-neutral-700">→</span>
                      <span className={filesB.length > 0 ? "text-green-400" : "text-neutral-600"}>
                        3. TIFF {filesB.length > 0 ? "OK" : "..."}
                      </span>
                    </div>
                  )}
                </>
              ...
              {!isFullscreen && (
                <div className="absolute top-4 right-4 flex flex-col items-end gap-2 z-50">
                  <div className="flex items-center gap-2">
                    <div
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-full shadow-lg text-sm border pointer-events-none ${viewMode === "diff" ? "bg-red-900/60 border-red-500/30 text-red-300" : viewMode === "B" ? "bg-green-900/60 border-green-500/30 text-green-300" : "bg-blue-900/60 border-blue-500/30 text-blue-300"}`}
                    >
                      {viewMode === "diff" ? (
                        <>
                          <FileDiff size={14} /> 差分
                    ...
                    <>
                      ...
                      <div className="relative z-50 bg-neutral-800/95 backdrop-blur-md border border-white/[0.10] rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] p-4 text-sm min-w-64">
                        ...
                        <div className="space-y-1.5 text-neutral-300">
                          ...
                          <div className="flex justify-between">
                            <span className="text-neutral-600">Ctrl+Space</span>
                            <span>差分表示トグル</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-neutral-600">↑ / ↓</span>
                            <span>{compareMode === "pdf-pdf" ? "ページ移動" : "ファイル選択"}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-neutral-600">J / K</span>
                            <span>差分ファイル移動</span>
                          </div>
                          {(compareMode === "psd-psd" || compareMode === "psd-tiff") && (
                            <div className="flex justify-between">
                          ...
                          <div className="flex justify-between">
                            <span className="text-neutral-600">Ctrl+0</span>
                            <span>全体表示に戻す</span>
                          </div>
                          {viewMode === "diff" && (
                            <div className="flex justify-between">
                              <span className="text-neutral-600">ホイール</span>
                              <span>ページ切替</span>
                            </div>
                          )}
                          <div className="flex justify-between">
                            <span className="text-neutral-600">ドラッグ</span>
                            <span>パン移動（拡大時）</span>
                          </div>
                          <div className="border-t border-white/[0.06] my-2" />
                  ...
                  {compareMode === "psd-tiff" && currentPair.hasDiff && (
                    <div className="bg-orange-900/70 text-orange-300 px-3 py-1.5 rounded-lg shadow-lg text-sm font-semibold pointer-events-none border border-orange-500/20">
                      差分可能性: {currentPair.diffProbability}%
                    </div>
                  )}
                  {currentPair.hasDiff && currentMarkers.length > 0 && (
                    <div className="bg-cyan-900/70 text-cyan-300 px-3 py-1.5 rounded-lg shadow-lg text-sm font-semibold flex items-center gap-1 pointer-events-none border border-cyan-500/20">
                      <Target size={14} /> {currentMarkers.length}箇所
                    </div>
                  )}
                  {/* 差分ガイドナビゲーション */}
                  {diffFileIndices.length > 0 && (
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-neutral-800/90 backdrop-blur-md rounded-lg border border-white/[0.08] shadow-lg">
                      ...
                      <span className="text-xs font-semibold tracking-wide text-red-400/70">
                        差分
                      </span>
                      ...
                      <span className="text-sm font-semibold min-w-[32px] text-center tabular-nums">
                        {diffNavPosition.current >= 0 ? (
                          <>
                            <span className="text-neutral-200">{diffNavPosition.current + 1}</span>
                            <span className="text-neutral-500">/</span>
                            <span className="text-neutral-500">{diffNavPosition.total}</span>
                          </>
                        ) : (
                          <>
                            <span className="text-neutral-500">--</span>
                ...
                <div className="absolute bottom-4 right-4 bg-neutral-800/90 backdrop-blur-md border border-white/[0.08] p-3 rounded-lg shadow-lg text-xs pointer-events-none">
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(to right, rgb(0,0,200), rgb(0,200,200), rgb(255,255,0), rgb(255,0,0))",
                      }}
                    />
                  </div>
          ...
          <>
            {initialModeSelect ? (
              <div className="flex flex-col items-center w-full max-w-2xl">
                <p className="text-neutral-100 text-2xl font-semibold mb-8 tracking-wide">
                  モードを選択
                </p>
                ...
                <div className="flex gap-4 mb-8 w-full">
                  ...
                  >
                    ...
                    />
                    <span className="text-lg font-semibold text-neutral-500 group-hover:text-green-400 transition-colors">
                      分割ビューアー
                    </span>
                  </button>
                </div>
                ...
                <p className="text-neutral-600 text-sm mb-4 tracking-wide">
                  比較するファイル形式を選んでください
                </p>
                <div className="grid grid-cols-2 gap-4 w-full">
                  ...
                  >
                    <FileImage
                      size={40}
                      className="mb-2 text-blue-400 group-hover:text-blue-300 transition-colors"
                    />
                  ...
                  >
                    <Palette
                      size={40}
                      className="mb-2 text-purple-400 group-hover:text-purple-300 transition-colors"
                    />
                  ...
                  >
                    <FileText
                      size={40}
                      className="mb-2 text-rose-400 group-hover:text-rose-300 transition-colors"
                    />
                    <p className="text-lg font-semibold text-rose-400 group-hover:text-rose-300 transition-colors">
                      PDF
                    </p>
                  ...
                  >
                    <Shuffle
                      size={40}
                      className="mb-2 text-orange-400 group-hover:text-orange-300 transition-colors"
                    />
                    <p className="text-lg font-semibold text-orange-400 group-hover:text-orange-300 transition-colors">
                      混合
                    </p>
                  ...
                  >
                    <Type
                      size={40}
                      className="mb-2 text-teal-400 group-hover:text-teal-300 transition-colors"
                    />
                    <p className="text-lg font-semibold text-teal-400 group-hover:text-teal-300 transition-colors">
                      テキスト照合
                    </p>
      ...
      {/* Status bar */}
      <div
        className={`bg-neutral-900 border-t border-white/[0.06] flex items-center px-4 text-xs text-neutral-600 justify-between shrink-0 overflow-hidden transition-all duration-300 ease-in-out ${isFullscreen || fullscreenTransitioning ? "h-0 opacity-0 border-t-0" : "h-8 opacity-100"}`}
      >
        <div className="flex items-center gap-3">
          <span>#{selectedIndex + 1}</span>
          {currentPair?.nameA && <span className="text-neutral-500">{currentPair.nameA}</span>}
          {compareMode === "pdf-pdf" && currentPair?.totalPages && currentPair.totalPages > 1 && (
            <span className="text-rose-400">
              P.{currentPage}/{currentPair.totalPages}
            </span>
          )}
          {currentPair?.hasDiff && (
            <>
        ...
        <div className="flex items-center gap-3">
          <span
            className={`px-2 py-0.5 rounded-md ${compareMode === "psd-tiff" ? "bg-[rgba(196,164,124,0.08)] text-orange-400" : compareMode === "psd-psd" ? "bg-[rgba(164,140,196,0.08)] text-purple-400" : compareMode === "pdf-pdf" ? "bg-[rgba(196,140,156,0.08)] text-rose-400" : "bg-[rgba(124,156,196,0.08)] text-blue-400"}`}
          >
            {compareMode === "psd-tiff"
              ? "PSD-TIFF"
              : compareMode === "psd-psd"
                ? "PSD-PSD"
                : compareMode === "pdf-pdf"
                  ? `PDF-PDF ${preloadProgress.total > 0 ? `(${preloadProgress.loaded}/${preloadProgress.total})` : ""}`
                  : "TIFF-TIFF"}
          </span>
        </div>
      </div>
