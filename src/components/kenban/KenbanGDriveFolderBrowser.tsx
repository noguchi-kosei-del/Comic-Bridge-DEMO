...
const GDriveFolderBrowser: React.FC<Props> = ({ isOpen, onClose, onJsonSelect }) => {
  ...
  const loadRoot = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Gドライブにアクセスできません。共有ドライブがマウントされているか確認してください。",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  // 再帰的にJSONファイルを検索
  const searchJsonFiles = useCallback(
    async (dirPath: string, query: string, results: SearchResult[], depth: number = 0) => {
    ...
    try {
      ...
      if (selectionRanges && selectionRanges.length >= 1) {
        const ranges: SelectionRange[] = selectionRanges
          .filter((r: { bounds?: CropBounds }) => r.bounds)
          .map(
            (r: { label?: string; bounds: CropBounds; blurRadius?: number; savedAt?: string }) => ({
              label: r.label || "名称未設定",
              bounds: r.bounds,
              blurRadius: r.blurRadius,
              savedAt: r.savedAt,
    ...
    >
      <div className="bg-neutral-800 rounded-lg shadow-xl w-[500px] max-h-[70vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.04]">
          ...
          <button onClick={onClose} className="p-1 hover:bg-neutral-600 rounded transition-colors">
            <X size={18} />
          </button>
        </div>
              ...
              <div className="space-y-1">
                ...
                {searchResults.map((result, index) => (
                  ...
                  >
                    ...
                    {result.folderPath && (
                      <span className="text-xs text-neutral-500 ml-5 truncate">
                        {result.folderPath}
                      </span>
                    )}
                  </div>
        ...
        >
          <div className="bg-neutral-800 rounded-lg shadow-xl w-[350px] max-h-[50vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-2">
              <div className="text-xs text-neutral-500 px-2 mb-2">{pendingSelection.fileName}</div>
