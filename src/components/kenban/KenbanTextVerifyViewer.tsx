...
import {
  ...
  buildUnifiedDiff,
  CHUNK_BREAK,
  getMemoTextFromEntry,
  getPsdTextFromEntry,
  reconstructMemoFromEntries,
...
export default function TextVerifyViewer({
  ...
  const layerDiffMap = useMemo((): (ExtractedTextLayer & { isLineBreakOnly: boolean })[] => {
    ...
    for (let li = 0; li < currentPage.extractedLayers.length; li++) {
      const lines = normalizeTextForComparison(currentPage.extractedLayers[li].text)
        .split("\n")
        .filter(Boolean);
    ...
    return currentPage.extractedLayers
      .map((layer, idx) => {
  ...
  const diffCount = useMemo(
    () => unifiedEntries.filter((e) => e.type === "diff" || e.type === "linebreak").length,
    [unifiedEntries],
  );
  ...
  useEffect(() => {
    if (prevIndexRef.current !== currentIndex) {
      ...
      if (editingBlockIndex !== null) {
        const origMemoText = getMemoTextFromEntry(unifiedEntries[editingBlockIndex]);
        if (editText !== origMemoText) {
          const newMemoText = reconstructMemoFromEntries(
            unifiedEntries,
            editingBlockIndex,
            editText,
          );
  ...
  const renderDiffText = useCallback(
    (parts: DiffPart[], diffType: string, missingLabel: string) => {
      ...
      const fullLineStyle: React.CSSProperties = isRemove
        ? {
            background: "rgba(194,90,90,0.10)",
            color: "#8a2020",
            borderLeft: "2px solid rgba(194,90,90,0.5)",
        ...
        : {
            ...
            color: "#1a6030",
            borderLeft: "2px solid rgba(60,150,80,0.45)",
            borderRadius: "0 2px 2px 0",
            padding: "1px 4px 1px 6px",
            flex: 1,
          };
          ...
          return (
            <span key={i} className="flex items-center py-1.5" style={{ display: "flex" }}>
              <span
                ...
                style={{
                  background: "linear-gradient(to right, transparent, var(--tv-rule), transparent)",
                }}
              />
            </span>
          );
          ...
          <div className="flex flex-col items-center w-full max-w-3xl px-8">
            ...
            <div className="flex gap-4 w-full">
              ...
              >
                <FolderOpen
                  size={36}
                  className={`mb-3 ${dragOverSide === "textVerifyPsd" ? "text-teal-400" : "text-neutral-600"}`}
                />
                <p
                  className={`text-sm font-medium ${dragOverSide === "textVerifyPsd" ? "text-teal-300" : "text-neutral-500"}`}
                >
                  PSDフォルダ
                </p>
              ...
              >
                {memoRaw ? (
                  ...
                  <>
                    <FileText
                      size={36}
                      className={`mb-3 ${dragOverSide === "textVerifyMemo" ? "text-teal-400" : "text-neutral-600"}`}
                    />
                    <p
                      className={`text-sm font-medium ${dragOverSide === "textVerifyMemo" ? "text-teal-300" : "text-neutral-500"}`}
                    >
                      テキストメモ
                    </p>
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/[0.04] bg-surface-raised">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-[10px] text-neutral-500 min-w-[36px] text-center">
              {Math.round(zoom * 100)}%
            </span>
            <button
            ...
            {layerDiffMap.length > 0 && (
              <button
                ...
                onMouseEnter={(e) => {
                  if (!showHighlights) e.currentTarget.style.color = "rgb(212,212,216)";
                }}
                onMouseLeave={(e) => {
                  if (!showHighlights) e.currentTarget.style.color = "rgb(115,115,130)";
                }}
              >
          ...
          {currentPage?.status === "error" && (
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-neutral-500">
                {currentPage.errorMessage || "エラーが発生しました"}
              </span>
            </div>
          )}
                ...
                >
                  {layerDiffMap.map((layer, i) => (
                    <rect
                      ...
                      fill={
                        layer.isLineBreakOnly ? "rgba(59,130,246,0.12)" : "rgba(194,90,90,0.15)"
                      }
                      stroke={
                        layer.isLineBreakOnly ? "rgba(59,130,246,0.45)" : "rgba(194,90,90,0.5)"
                      }
          ...
          {currentPage?.status === "done" && (
            <div className="absolute top-3 right-3 pointer-events-none">
              {hasDiff ? (
                <AlertTriangle
                  size={28}
                  className="text-red-400 drop-shadow-[0_1px_4px_rgba(248,113,113,0.5)]"
                />
              ) : (
                <CheckCircle
                  size={28}
                  className="text-green-400 drop-shadow-[0_1px_4px_rgba(74,222,128,0.5)]"
                />
              )}
            </div>
          )}
      ...
      <div
        className="tv-panel w-[420px] shrink-0 flex flex-col overflow-hidden select-text"
        style={{ background: "var(--tv-paper)", borderLeft: "1px solid var(--tv-rule-strong)" }}
      >
        {/* アクセントライン */}
        <div
          className="h-[2px] shrink-0"
          style={{ background: "linear-gradient(90deg, var(--tv-accent), transparent)" }}
        />
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (canUndo) {
                      ...
                      e.currentTarget.style.background = "var(--tv-accent-wash)";
                    }
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.color = "var(--tv-ink-tertiary)";
                  ...
                  <button
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = hasUnsavedChanges
                        ? "var(--tv-accent-wash)"
                        : "transparent";
                      e.currentTarget.style.color = hasUnsavedChanges
                        ? "var(--tv-accent)"
                        : "var(--tv-ink-tertiary)";
                    }}
                  >
        ...
        {!memoRaw && (
          <div
            ...
            style={{
              background: dragOverSide === "textVerifyMemo" ? "var(--tv-accent-wash)" : undefined,
            }}
          >
            <div
              className="text-center space-y-4 border border-dashed rounded-lg px-8 py-14 w-full transition-colors duration-200"
              style={{
                borderColor:
                  dragOverSide === "textVerifyMemo" ? "var(--tv-accent)" : "var(--tv-rule-strong)",
              }}
            >
              <FileText
                size={28}
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "rgba(58,112,112,0.14)";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "var(--tv-accent-wash)";
                }}
        ...
        {memoRaw && currentPage && (
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div
              className="px-3 py-1.5 flex items-center gap-3 shrink-0"
              style={{ background: "var(--tv-header)", borderBottom: "1px solid var(--tv-rule)" }}
            >
              {/* セグメントコントロール */}
              <div
              ...
              >
                {(
                  [
                    ...
                    {
                      key: "split" as const,
                      icon: <SplitSquareVertical size={11} />,
                      label: "分割",
                    },
                  ] as const
                ).map((tab) => {
              ...
              {viewMode === "unified" && diffPageIndices.length > 0 && (
                ...
                >
                  ...
                  <span
                    className="text-[11px] font-semibold min-w-[28px] text-center tabular-nums"
                    style={{ color: "var(--tv-ink-secondary)" }}
                  >
                    {globalDiffPos.current >= 0 ? (
                      <>
                        {globalDiffPos.current + 1}
                        <span style={{ color: "var(--tv-ink-tertiary)", margin: "0 1px" }}>/</span>
                        {globalDiffPos.total}
                      </>
                    ) : (
                      <>
            ...
            {isFullEditing ? (
              ...
              >
                {currentPage.memoShared && (
                  <div
                    className="mb-2 px-2.5 py-1.5 rounded text-[10px] flex items-center gap-1.5"
                    style={{
                      color: "var(--tv-ink-secondary)",
                  ...
                  >
                    ...
                    このメモは P{currentPage.memoSharedGroup.join(", P")}{" "}
                    で共有されています。変更は全ページに反映されます。
                  </div>
                )}
                <textarea
                  ref={textareaRef}
                ...
                {currentPage.status === "loading" ? (
                  ...
                  <div className="space-y-2">
                    {currentPage.extractedLayers.map((layer, i) => {
                      const layerLines = normalizeTextForComparison(layer.text)
                        .split("\n")
                        .filter(Boolean);
                      const hasLayerDiff =
                        memoLinesSet.size > 0 && layerLines.some((l) => !memoLinesSet.has(l));
                      const allInMemo =
                        memoLinesSet.size > 0 &&
                        layerLines.length > 0 &&
                        layerLines.every((l) => memoLinesSet.has(l));
                      return (
                        <div
                          key={i}
                          ...
                          style={{
                            ...
                            borderLeft: hasLayerDiff
                              ? "3px solid rgba(194,90,90,0.5)"
                              : allInMemo
                                ? "3px solid rgba(60,150,80,0.4)"
                                : undefined,
                          }}
                        >
                          <div className="flex items-start gap-1.5">
                            {hasLayerDiff && (
                              <AlertTriangle
                                ...
                                style={{ color: "rgba(194,90,90,0.7)" }}
                              />
                            )}
                            {allInMemo && (
                              <CheckCircle
                                size={10}
                                className="shrink-0 mt-1.5"
                                style={{ color: "rgba(60,150,80,0.7)" }}
                              />
                            )}
                            <div
                              className="flex-1 text-[13px] whitespace-pre-wrap break-all leading-[1.8]"
              ...
              <div ref={unifiedScrollRef} className="flex-1 overflow-y-auto px-4 py-3">
                {currentPage.memoShared && (
                  <div
                    className="mb-2 px-2.5 py-1 rounded text-[10px]"
                    style={{
                      color: "var(--tv-ink-tertiary)",
                      background: "rgba(108,168,168,0.06)",
                      border: "1px solid rgba(108,168,168,0.12)",
                    }}
                  >
                ...
                {currentPage.status === "loading" ? (
                  ...
                  <div
                    className="text-[13px] whitespace-pre-wrap break-all leading-[1.8]"
                    style={{ color: "var(--tv-ink)" }}
                  >
                      ...
                      return unifiedEntries.map((entry, i) => {
                          ...
                          return (
                            <div key={i} className="flex items-center py-1.5">
                              <div
                                ...
                                style={{
                                  background:
                                    "linear-gradient(to right, transparent, var(--tv-rule), transparent)",
                                }}
                              />
                            </div>
                          );
                        ...
                        if (entry.type === "match") {
                          if (editingBlockIndex === i) {
                            return (
                              <div key={i}>
                                {currentPage.memoShared && (
                                  <div
                                    className="mb-1 px-2 py-1 rounded text-[10px] flex items-center gap-1"
                                    style={{
                                      color: "var(--tv-ink-secondary)",
                                      background: "rgba(200,160,60,0.08)",
                                  ...
                                  >
                                    <AlertTriangle
                                      size={9}
                                      style={{ color: "rgba(200,160,60,0.7)" }}
                                    />
                          ...
                          return (
                            <div
                              ...
                              onDoubleClick={(e) => {
                                cursorOffsetRef.current = getClickOffset(e.currentTarget);
                                const t = getMemoTextFromEntry(entry);
                                if (t !== null) {
                              ...
                              style={{ padding: "0 1px" }}
                              onMouseEnter={(e) => {
                                e.currentTarget.style.background = "rgba(58,112,112,0.04)";
                              }}
                              onMouseLeave={(e) => {
                                e.currentTarget.style.background = "transparent";
                              }}
                        ...
                        if (entry.type === "linebreak") {
                          ...
                          if (editingBlockIndex === i) {
                            return (
                              <div key={i} className="my-1">
                                <div
                                  className="px-2.5 py-1 rounded-t-md text-[12px] leading-[1.7] whitespace-pre-wrap break-all"
                                  style={{
                                    background: "rgba(59,130,246,0.06)",
                                    color: "var(--tv-ink-secondary)",
                                    border: "2px solid var(--tv-accent)",
                                    borderBottom: "1px dashed var(--tv-rule)",
                                  }}
                                >
                                  <span
                                    className="text-[9px] font-semibold tracking-wide select-none mr-1.5"
                                ...
                                <div
                                  className="px-2.5 py-0.5 flex items-center gap-1.5"
                                  style={{
                                    borderLeft: "2px solid var(--tv-accent)",
                                    borderRight: "2px solid var(--tv-accent)",
                                    background: "rgba(50,130,70,0.04)",
                                    borderTop: "1px dashed var(--tv-rule)",
                                  }}
                                >
                                  <span
                                  ...
                                  {currentPage.memoShared && (
                                    <span
                                      ...
                                      style={{ color: "rgba(200,160,60,0.7)" }}
                                    >
                                      <AlertTriangle size={8} />P
                                      {currentPage.memoSharedGroup.join(", P")} 共有
                                    </span>
                                  )}
                                </div>
                                <textarea
                          ...
                          return (
                            ...
                            >
                              <div
                                className="px-2.5 pt-1 pb-0"
                                style={{ borderBottom: "1px solid rgba(59,130,246,0.12)" }}
                              >
                                <span
                                  className="flex items-center gap-1 text-[9px] font-semibold tracking-wide select-none"
                                  style={{ color: "rgba(59,130,246,0.6)" }}
                                >
                              ...
                              <div className="px-2.5 py-1 break-all">
                                ...
                                <span
                                  className="inline-flex items-center gap-0.5 text-[9px] font-semibold tracking-wide select-none ml-1.5 align-baseline"
                              ...
                              <div
                                className="px-2.5 py-1 break-all"
                                style={{ borderTop: "1px dashed rgba(59,130,246,0.15)" }}
                              >
                            ...
                            <div key={i} className="my-1">
                              {psdRef && (
                                ...
                                >
                                  <span
                                    className="text-[9px] font-semibold tracking-wide select-none mr-1.5"
                                    style={{ color: "rgba(160,70,70,0.6)" }}
                                  >
                                    PSD
                                  </span>
                                  {psdRef}
                        ...
                        return (
                          <div
                            ...
                            style={{
                              background: isCurrent
                                ? "rgba(210,170,80,0.10)"
                                : "var(--tv-paper-warm)",
                              border: `1px solid ${isCurrent ? "rgba(200,160,60,0.5)" : "var(--tv-rule)"}`,
                              boxShadow: isCurrent
                                ? "0 0 0 1px rgba(200,160,60,0.35), 0 1px 4px rgba(200,160,60,0.12)"
                                : undefined,
                            }}
                            onDoubleClick={
                              entry.memoParts
                          ...
                          >
                            ...
                            {entry.memoParts && (
                              <div
                                className="px-2.5 py-1 whitespace-pre-wrap break-all"
                                style={{
                                  borderTop: entry.psdParts
                                    ? "1px dashed var(--tv-rule)"
                                    : undefined,
                                }}
                              >
                                {entry.memoParts.map((p, mi) =>
                                  !p.value ? null : p.added ? (
                                    <span
                                      key={mi}
                                      style={{
                                        background: "rgba(60,150,80,0.16)",
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-3">
                    {currentPage.status === "loading" ? (
                      ...
                      >
                        {diffResult
                          ? renderDiffText(diffResult.psd, "tv-diff-remove", "PSDのみ")
                          : currentPage.extractedText}
                      </div>
                    )}
                  </div>
                    ...
                    {isSplitEditing ? (
                      <div className="flex flex-col h-full">
                        {currentPage.memoShared && (
                          ...
                          >
                            <AlertTriangle size={10} style={{ color: "rgba(200,160,60,0.7)" }} />P
                            {currentPage.memoSharedGroup.join(", P")} で共有 — 変更は全ページに反映
                          </div>
                        )}
                        <textarea
                          ref={textareaRef}
                      ...
                      >
                        {diffResult
                          ? renderDiffText(diffResult.memo, "tv-diff-add", "メモのみ")
                          : currentPage.memoText}
                      </div>
                    )}
                  </div>
