...
export default function ScreenshotEditor({ imageData, onClose }: ScreenshotEditorProps) {
  ...
  const [currentLeaderLine, setCurrentLeaderLine] = useState<{
    startX: number;
    startY: number;
    endX: number;
    endY: number;
  ...
  // 引き出し線テキスト入力
  const [leaderTextInput, setLeaderTextInput] = useState<{
  ...
  const annotations = history
    .filter((h): h is { type: "annotation"; annotation: Annotation } => h.type === "annotation")
    .map((h) => h.annotation);
  const cropRegion =
    history.filter((h): h is { type: "crop"; region: CropRegion } => h.type === "crop").pop()
      ?.region || null;
  ...
  const displayRegion = cropRegion || {
    x: 0,
    y: 0,
      ...
      if (ann.type === "rect" && ann.rect) {
          ...
          if (ann.label) {
            if (ann.label.vertical) {
              ...
              const labelWidth = Math.max(
                ...lines.map((line) => measureTextWidth(line, ann.label!.fontSize)),
              );
        ...
        if (ann.text.vertical) {
          ...
          const maxWidth = Math.max(
            ...lines.map((line) => measureTextWidth(line, ann.text!.fontSize)),
          );
  ...
  const getClickedPart = useCallback(
    (x: number, y: number, ann: Annotation): "rect" | "label" | null => {
  ...
  const hitTestAnnotations = useCallback(
  ...
  const getResizeHandleAtPoint = useCallback(
    (x: number, y: number, bounds: Bounds): ResizeHandle => {
      ...
      // 四隅（優先度高）
      // 左上
      if (Math.abs(x - bounds.x) < handleHitSize && Math.abs(y - bounds.y) < handleHitSize)
        return "tl";
      // 右上
      if (
        Math.abs(x - (bounds.x + bounds.width)) < handleHitSize &&
        Math.abs(y - bounds.y) < handleHitSize
      )
        return "tr";
      // 左下
      if (
        Math.abs(x - bounds.x) < handleHitSize &&
        Math.abs(y - (bounds.y + bounds.height)) < handleHitSize
      )
        return "bl";
      // 右下
      if (
      ...
      // 下辺中央
      if (
        Math.abs(x - centerX) < edgeHitSize &&
        Math.abs(y - (bounds.y + bounds.height)) < edgeHitSize
      )
        return "b";
      // 左辺中央
      ...
      // 右辺中央
      if (
        Math.abs(x - (bounds.x + bounds.width)) < edgeHitSize &&
        Math.abs(y - centerY) < edgeHitSize
      )
        return "r";
    ...
    (
      ...
      if (ann.type === "rect" && ann.rect) {
        ...
        if (ann.leaderLine && part) {
          ...
          const getLabelBounds = (label: {
            ...
            if (label.vertical) {
              ...
              const labelWidth =
                Math.max(...lines.map((line) => measureTextWidth(line, label.fontSize))) || 20;
          ...
          if (part === "rect") {
            ...
            const endPoint = getClosestPointOnRectEdge(
              newLabelBounds.x,
              newLabelBounds.y,
              newLabelBounds.width,
              newLabelBounds.height,
              rectCenterX,
              rectCenterY,
            );
        ...
        if (ann.text.vertical) {
          ...
          const maxChars = Math.max(...columns.map((col) => [...col].length));
          const newFontSize =
            maxChars > 0 ? newBounds.height / (maxChars * 1.1) : ann.text.fontSize;
  ...
  const redrawCanvas = useCallback(() => {
    ...
    if (leaderDrawPhase === "leader" && pendingRectAnnotation) {
      ...
      if (currentLeaderLine) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          currentLeaderLine.startX - displayRegion.x,
          currentLeaderLine.startY - displayRegion.y,
        );
        ctx.lineTo(
          currentLeaderLine.endX - displayRegion.x,
          currentLeaderLine.endY - displayRegion.y,
        );
        ctx.stroke();
      }
    }
    ...
    if (selectedAnnotation && tool === "select") {
      ...
      if (
        ...
        selectedAnnotation.leaderLine
      ) {
        // 枠線部分のバウンディングボックス（青）
        const rectX =
          selectedAnnotation.rect.width >= 0
            ? selectedAnnotation.rect.x
            : selectedAnnotation.rect.x + selectedAnnotation.rect.width;
        const rectY =
          selectedAnnotation.rect.height >= 0
            ? selectedAnnotation.rect.y
            : selectedAnnotation.rect.y + selectedAnnotation.rect.height;
        ...
        rectHandles.forEach((h) => {
          ...
          ctx.fillRect(
            h.x - handleSize / 2 - borderSize,
            h.y - handleSize / 2 - borderSize,
            handleSize + borderSize * 2,
            handleSize + borderSize * 2,
          );
          // 塗りつぶし
        ...
        if (selectedAnnotation.label) {
          ...
          if (selectedAnnotation.label.vertical) {
            ...
            labelWidth =
              Math.max(
                ...lines.map((line) => measureTextWidth(line, selectedAnnotation.label!.fontSize)),
              ) || 20;
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (tool === "select") {
      ...
      // ラベルのバウンディングボックスを計算するヘルパー
      const getLabelBoundsForResize = (label: {
        x: number;
        y: number;
        content: string;
        fontSize: number;
      ...
      if (hitAnn) {
        ...
        if (hitAnn.type === "rect" && hitAnn.rect && hitAnn.leaderLine) {
          ...
          const rectY =
            hitAnn.rect.height >= 0 ? hitAnn.rect.y : hitAnn.rect.y + hitAnn.rect.height;
          const rectBounds: Bounds = {
            x: rectX,
            y: rectY,
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isResizing && originalAnnotation && dragStart && resizeHandle) {
      ...
      if (
        originalAnnotation.type === "rect" &&
        originalAnnotation.rect &&
        originalAnnotation.leaderLine &&
        resizingPart
      ) {
        if (resizingPart === "rect") {
          const rectX =
            originalAnnotation.rect.width >= 0
              ? originalAnnotation.rect.x
              : originalAnnotation.rect.x + originalAnnotation.rect.width;
          const rectY =
            originalAnnotation.rect.height >= 0
              ? originalAnnotation.rect.y
              : originalAnnotation.rect.y + originalAnnotation.rect.height;
          bounds = {
            x: rectX,
            y: rectY,
          ...
          const labelWidth =
            Math.max(
              ...lines.map((line) => measureTextWidth(line, originalAnnotation.label!.fontSize)),
            ) || 20;
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    ...
    if (currentAnnotation) {
      ...
      if (currentAnnotation.type === "rect" && currentAnnotation.rect) {
        if (
          Math.abs(currentAnnotation.rect.width) > 5 &&
          Math.abs(currentAnnotation.rect.height) > 5
        ) {
          // 引き出し線モードの場合
    ...
    if (currentCropSelection && tool === "crop") {
      ...
      const x1 = Math.min(
        currentCropSelection.x,
        currentCropSelection.x + currentCropSelection.width,
      );
      const y1 = Math.min(
        currentCropSelection.y,
        currentCropSelection.y + currentCropSelection.height,
      );
      const x2 = Math.max(
        currentCropSelection.x,
        currentCropSelection.x + currentCropSelection.width,
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        ...
        e.preventDefault();
        setHistory((prev) =>
          prev.filter((h) => !(h.type === "annotation" && h.annotation.id === selectedId)),
        );
        setSelectedId(null);
      }
    };
    ...
    if (textInput && textValue.trim()) {
      if (editingAnnotationId) {
        ...
        const updatedHistory = history.map((h) => {
          if (
            h.type === "annotation" &&
            h.annotation.id === editingAnnotationId &&
            h.annotation.text
          ) {
            return {
              ...h,
          ...
          <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/[0.04]">
            <div className="flex items-center gap-2">
              ...
              {(tool === "rect" || tool === "pen" || tool === "text") && (
                <div
                  ref={colorPaletteRef}
                  className="flex items-center gap-0.5 bg-neutral-900 rounded p-0.5 relative"
                >
                  {(tool === "rect" || tool === "pen") && (
                    <button
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3 text-xs px-2 py-1 bg-neutral-800 rounded border border-white/[0.06]">
                <span className="flex items-center gap-1">
                  <kbd className="px-1 py-0.5 bg-neutral-700 rounded text-neutral-200 font-mono text-[10px] border border-neutral-500">
                    Space
                  </kbd>
                          ...
                          : tool === "select"
                            ? isResizing
                              ? resizeHandle === "tl" || resizeHandle === "br"
                                ? "nwse-resize"
                                : resizeHandle === "tr" || resizeHandle === "bl"
                                  ? "nesw-resize"
                                  : resizeHandle === "t" || resizeHandle === "b"
                                    ? "ns-resize"
                                    : resizeHandle === "l" || resizeHandle === "r"
                                      ? "ew-resize"
                                      : "nwse-resize"
                              : isMoving
                                ? "move"
