...
const modeAccent: Record<string, { text: string; bg: string; border: string }> = {
  "tiff-tiff": {
    text: "text-blue-400",
    bg: "bg-[rgba(124,156,196,0.12)]",
    border: "border-[rgba(124,156,196,0.20)]",
  },
  "psd-psd": {
    text: "text-purple-400",
    bg: "bg-[rgba(164,140,196,0.12)]",
    border: "border-[rgba(164,140,196,0.20)]",
  },
  "pdf-pdf": {
    text: "text-rose-400",
    bg: "bg-[rgba(196,140,156,0.12)]",
    border: "border-[rgba(196,140,156,0.20)]",
  },
  "psd-tiff": {
    text: "text-orange-400",
    bg: "bg-[rgba(196,164,124,0.12)]",
    border: "border-[rgba(196,164,124,0.20)]",
  },
  "text-verify": {
    text: "text-teal-400",
    bg: "bg-[rgba(108,168,168,0.12)]",
    border: "border-[rgba(108,168,168,0.20)]",
  },
};
...
export default function Sidebar({
  ...
  // Derived values
  const filteredPairs = filterDiffOnly
    ? pairs.filter((p) => (p.status === "done" || p.status === "checked") && p.hasDiff)
    : pairs;
  ...
  return (
    <div
      className={`bg-neutral-800 border-r border-white/[0.04] shadow-[2px_0_12px_rgba(0,0,0,0.15)] flex flex-col shrink-0 overflow-hidden transition-all duration-300 ease-in-out ${isFullscreen || fullscreenTransitioning ? "w-0 opacity-0 border-r-0" : sidebarCollapsed ? "w-10 opacity-100" : "w-72 opacity-100"}`}
    >
      {/* Collapse button */}
      <div
        className={`flex items-center border-b border-white/[0.04] ${sidebarCollapsed ? "justify-center p-2" : "justify-end px-2 py-1"}`}
      >
        <button
      ...
      {!sidebarCollapsed && (
        <>
          ...
          {appMode === "diff-check" ? (
              ...
              <div className="p-3 border-b border-white/[0.04]">
                ...
                <div className="flex gap-1 mb-2">
                  <button
                    ...
                    className={`flex-1 text-xs py-1.5 rounded-md transition-all ${modeButtonClass("tiff-tiff")}`}
                  >
                    TIFF
                  </button>
                  <button
                    onClick={() => handleModeChange("psd-psd")}
                    className={`flex-1 text-xs py-1.5 rounded-md transition-all ${modeButtonClass("psd-psd")}`}
                  >
                    PSD
                  </button>
                  <button
                    onClick={() => handleModeChange("pdf-pdf")}
                    className={`flex-1 text-xs py-1.5 rounded-md transition-all ${modeButtonClass("pdf-pdf")}`}
                  >
                    PDF
                  </button>
                  <button
                    onClick={() => handleModeChange("psd-tiff")}
                    className={`flex-1 text-xs py-1.5 rounded-md transition-all ${modeButtonClass("psd-tiff")}`}
                  >
                    混合
                  </button>
                </div>
                <button
                  ...
                  className={`w-full flex items-center justify-center gap-1.5 text-xs py-1.5 rounded-md transition-all mb-3 ${modeButtonClass("text-verify")}`}
                >
                  <Type size={12} />
                  テキスト照合
                </button>
                ...
                {compareMode === "text-verify" ? (
                      ...
                      <div className="flex gap-1">
                        ...
                        >
                          ...
                          <span className="flex-1 text-left truncate">
                            {textVerifyMemoRaw ? "メモ読込済" : "テキストメモ"}
                          </span>
                        </button>
                        <button
                  ...
                  <>
                    <div className="flex gap-2">
                      <div
                        className={`flex-1 relative rounded-md transition-colors ${dragOverSide === "A" ? "ring-1 ring-blue-400/50 bg-blue-900/20" : ""}`}
                      ...
                      >
                        <button
                          ...
                          className="w-full text-center py-2 bg-neutral-700 hover:bg-neutral-600 rounded-md cursor-pointer text-xs transition-colors"
                        >
                          {modeLabels.a} ({filesA.length})
                        </button>
                        {dragOverSide === "A" && (
                          <div className="absolute inset-0 flex items-center justify-center bg-blue-600/60 rounded-md text-white text-xs font-medium pointer-events-none">
                            ドロップ
                          </div>
                        )}
                      </div>
                      <div
                        className={`flex-1 relative rounded-md transition-colors ${dragOverSide === "B" ? "ring-1 ring-green-400/50 bg-green-900/20" : ""}`}
                        onDragOver={handleDragOver}
                      ...
                      >
                        ...
                        {dragOverSide === "B" && (
                          <div className="absolute inset-0 flex items-center justify-center bg-green-600/60 rounded-md text-white text-xs font-medium pointer-events-none">
                            ドロップ
                          </div>
                        )}
                      </div>
                    ...
                    <div className="mt-1.5 text-[10px] text-neutral-600 text-center tracking-wide">
                      ファイル/フォルダをドロップ可能
                    </div>
                      ...
                      <div className="mt-2 space-y-1 text-xs">
                        <div className="flex items-center gap-1.5 px-1">
                          ...
                          <span className="text-neutral-300 truncate">
                            {filesA.length > 0
                              ? (filesA[0] as FileWithPath).filePath
                                  ?.split(/[/\\]/)
                                  .slice(-2, -1)[0] || filesA[0].name
                              : "-"}
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5 px-1">
                          ...
                          <span className="text-neutral-300 truncate">
                            {filesB.length > 0
                              ? (filesB[0] as FileWithPath).filePath
                                  ?.split(/[/\\]/)
                                  .slice(-2, -1)[0] || filesB[0].name
                              : "-"}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    >
                      ...
                      >
                        <HardDrive size={14} />
                        {cropBounds ? (
                          <span className="text-green-400">JSON OK</span>
                        ) : (
                          <span className="text-orange-400">Gドライブ</span>
                        )}
                      </button>
                      {dragOverSide === "json" && (
                        <div className="absolute inset-0 flex items-center justify-center bg-orange-600/60 rounded-md text-white text-xs font-medium pointer-events-none">
                          ドロップ
                        </div>
                      )}
                    </div>
                ...
                {compareMode !== "text-verify" && stats.pending > 0 && (
                  <div className="mt-2 w-full bg-white/[0.06] rounded-full h-1">
                    <div
                      className="bg-action h-1 rounded-full transition-all shadow-[0_0_6px_rgba(107,138,255,0.3)]"
              ...
              {compareMode === "text-verify" ? (
                ...
                <div ref={textVerifyFileListRef} className="flex-1 overflow-y-auto">
                  ...
                  {textVerifyPages.map((page, idx) => {
                    ...
                    const hasDiff = page.diffResult
                      ? page.diffResult.psd.some((d) => d.removed) ||
                        page.diffResult.memo.some((d) => d.added)
                      : false;
                    return (
                      <button
                        key={idx}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-xs truncate ${isSelected ? "text-teal-300" : "text-neutral-300"}`}
                          >
                            {page.fileName.replace(/\.psd$/i, "")}
                          </span>
                          {page.status === "loading" && (
                            <Loader2 size={12} className="text-teal-300 animate-spin shrink-0" />
                          )}
                          {page.status === "done" && !hasDiff && (
                          ...
                          {page.status === "pending" && !page.memoText && (
                ...
                <>
                  <div className="border-b border-white/[0.04]">
                    <button
                      ...
                      className="w-full px-3 py-2 flex items-center justify-between text-xs text-neutral-500 hover:bg-white/[0.04] transition-colors"
                    >
                    ...
                    {settingsOpen && (
                      <div className="px-3 pb-3 space-y-2">
                        <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-2 space-y-2">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={handleSelectPhotoshopExecutable}
                              className="flex-1 py-1 bg-[rgba(164,140,196,0.15)] hover:bg-[rgba(164,140,196,0.25)] text-purple-300 rounded-md text-xs transition-colors"
                            >
                              Photoshop.exe を選択
                            </button>
                            {photoshopPath && (
                              <button
                                onClick={handleClearPhotoshopExecutable}
                                className="px-2 py-1 bg-white/[0.06] hover:bg-white/[0.1] text-neutral-300 rounded-md text-xs transition-colors"
                              >
                                解除
                              </button>
                            )}
                        ...
                        <div className="flex gap-1 bg-neutral-950 rounded-lg p-0.5">
                          <button
                            onClick={() => setPairingMode("order")}
                            className={`flex-1 text-xs py-1 rounded-md transition-all ${pairingMode === "order" ? "bg-neutral-700 text-neutral-100 shadow-sm" : "text-neutral-500 hover:text-neutral-300"}`}
                          >
                            順番でペア
                          </button>
                          <button
                            onClick={() => setPairingMode("name")}
                            className={`flex-1 text-xs py-1 rounded-md transition-all ${pairingMode === "name" ? "bg-neutral-700 text-neutral-100 shadow-sm" : "text-neutral-500 hover:text-neutral-300"}`}
                          >
                            名前でペア
                          </button>
                        </div>
                        ...
                        <button
                          onClick={onClear}
                          className="w-full py-1 bg-red-900/30 hover:bg-red-900/50 text-red-400 rounded-md text-xs transition-colors"
                        >
                          クリア
                        </button>
                      </div>
                  ...
                  <div
                    ref={fileListRef}
                    className={`overflow-y-auto ${compareMode === "pdf-pdf" && currentPair?.totalPages && currentPair.totalPages > 1 ? "shrink-0 max-h-32" : "flex-1"}`}
                  >
                    {pairs.length === 0 && (
                      <div className="p-4 text-center text-neutral-600 text-xs">
                        ファイルをアップロード
                    ...
                    {stats.diff > 0 && (
                      <div className="px-3 py-2 bg-red-900/15 text-xs text-red-400 border-b border-white/[0.04]">
                        差分: {stats.diff}件
                      </div>
                    )}
                    ...
                    {filteredPairs.map((pair) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 border-b border-white/[0.03] transition-colors ${selectedIndex === pair.index ? "bg-[rgba(107,138,255,0.06)] border-l-2 border-l-action" : "hover:bg-white/[0.03]"}`}
                      >
                        <div className="flex items-center justify-between mb-0.5">
                          <div className="flex items-center gap-1 flex-1 min-w-0 mr-2">
                            ...
                            <span className="text-xs text-neutral-300 truncate">
                              {pair.nameA?.replace(/\.(psd|tiff?|pdf)/i, "") || "-"}
                            </span>
                          </div>
                          {(pair.status === "done" || pair.status === "checked") &&
                            (pair.hasDiff ? (
                              <AlertTriangle size={12} className="text-red-400 shrink-0" />
                            ) : (
                              <CheckCircle size={12} className="text-green-400 shrink-0" />
                            ))}
                          {(pair.status === "loading" || pair.status === "rendering") && (
                            <Loader2 size={12} className="text-action animate-spin shrink-0" />
                          )}
                          {pair.status === "pending" && pair.fileA && pair.fileB && (
                        ...
                        <div className="flex items-center gap-1 min-w-0">
                          ...
                          <span className="text-xs text-neutral-500 truncate">
                            {pair.nameB?.replace(/\.(psd|tiff?|pdf)/i, "") || "-"}
                          </span>
                        </div>
                      </button>
                    ))}
                    ...
                    currentPair.totalPages > 1 && (
                      <div className="flex-1 flex flex-col min-h-0 border-t border-white/[0.06]">
                        ...
                        <div ref={pageListRef} className="flex-1 overflow-y-auto">
                          {Array.from({ length: currentPair.totalPages }, (_, i) => i + 1).map(
                            (pageNum) => {
                              ...
                              return (
                                ...
                                >
                                  <span
                                    className={isCurrentPage ? "text-rose-300" : "text-neutral-500"}
                                  >
                                    P.{pageNum}
                                  </span>
                                  {pageData ? (
            ...
            <>
              <div className="p-3 border-b border-white/[0.04]">
                <div className="text-xs text-neutral-500 mb-3 tracking-wide">
                  フォルダ / PDF選択
                </div>
              ...
              <div ref={parallelFileListRef} className="flex-1 overflow-y-auto">
                ...
                {Array.from({ length: Math.max(parallelFilesA.length, parallelFilesB.length) }).map(
                  (_, idx) => {
