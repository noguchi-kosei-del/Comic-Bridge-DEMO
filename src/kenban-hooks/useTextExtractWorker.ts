...
import type {
  ...
  ReassignRequest,
  WorkerResponse,
} from "../kenban-workers/textExtractWorker";
...
function createWorker(): Worker {
  return new Worker(new URL("../kenban-workers/textExtractWorker.ts", import.meta.url), {
    type: "module",
  });
}
...
export function useTextExtractWorker() {
      ...
      return new Promise<WorkerExtractResult>((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
          ...
          if (pending) {
            ...
            console.error(
              `[TextExtractWorker] Request ${id} (pageIdx=${pageIdx}) timed out after ${WORKER_TIMEOUT_MS}ms`,
            );
  ...
  const reassignDiffs = useCallback(
    async (
